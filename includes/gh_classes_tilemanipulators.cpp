...
message TileManipulator::targethandler(Tile **board, const int board_h, const int board_w, Inventory &inventory)
{
    message mes;
    mes.text = "";
    mes.priority = 10;
...
Door::Door(int x, int y) : TileManipulator(x, y)
{
    this->locked = 0;
}
Door::Door(int x, int y, int locked) : TileManipulator(x, y)
{
    this->locked = locked;
}
Door::~Door()
...
message Door::targethandler(Tile **board, const int board_h, const int board_w, Inventory &inventory)
{
    mes.priority = 3;
    mes.text = "";
    // If door is idle
    if (close_countdown == -1)
    {
        ...
        else
        {
            ...
            if (inventory.use_item(_skeleton_key_name, false) == 0)
                had_key = true;
            else
            {
                ...
                {
                case 1:
                    if (inventory.use_item(_green_key_name, true) == 0)
                        had_key = true;
                    break;
                case 2:
                    if (inventory.use_item(_red_key_name, true) == 0)
                        had_key = true;
                    break;
                }
            }
...
message Switch::targethandler(Tile **board, const int board_h, const int board_w, Inventory &inventory)
{
    message mes;
